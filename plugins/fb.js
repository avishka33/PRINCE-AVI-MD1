...
const handler = async (m, {conn, args, command, usedPrefix}) => {
  ...
  if (!args[0].match(/www.facebook.com|fb.watch/g)) throw `*━━━━━━━━━━━━━━ 
  උදාහරණය;
  ━━━━━━━━━━━━━
  
  ${usedPrefix + command}* https://fb.watch//`;
  try {
    await m.reply(`*ඔයාගෙ විඩියො එක ඩව්න්ලෝඩ් වෙනො විනාඩි 2ත් 10ත් අතර කාලයක් ගතවිය හැක......*`);
    const d2ata = await facebook.v1(args[0]);
    let r2es = '';
    if (d2ata.urls && d2ata.urls.length > 0) {
      r2es = `${d2ata.urls[0]?.hd || d2ata.urls[1]?.sd || ''}`;
    }
    conn.sendFile(m.chat, r2es, 'error.mp4', `*🗡AVI🗡*`, m);
  } catch (err1) {
    try {
      ...
      try {
        ...
        conn.sendFile(m.chat, VIDEO, 'error.mp4', `*🗡AVI🗡*`, m);
      } catch (err2) {
        try {
          const ress = await fg.fbdl(args[0]);
          const urll = await ress.data[0].url;
          await conn.sendFile(m.chat, urll, 'error.mp4', '*🗡AVI🗡*', m);
        } catch (err3) {
          try {
            const res = await fbDownloader(args[0]);
            for (const result of res.download) {
              const ur = result.url;
              await conn.sendFile(m.chat, ur, 'error.mp4', '*🗡AVI🗡*', m);
            }
          } catch (err4) {
            try {
              ...
              await conn.sendFile(m.chat, url3, 'error.mp4', '*🗡AVI🗡*', m);
            } catch (err5) {
              try {
                const {result} = await facebookdl(args[0]).catch(async (_) => await facebookdlv2(args[0])).catch(async (_) => await savefrom(args[0]));
                for (const {url, isVideo} of result.reverse()) await conn.sendFile(m.chat, url, `facebook.${!isVideo ? 'bin' : 'mp4'}`, '*🗡AVI🗡*', m);
              } catch (err6) {
                throw `*[❗𝐈𝐍𝐅𝐎❗] 𝙴𝚁𝚁𝙾𝚁*`;
              }
            }
